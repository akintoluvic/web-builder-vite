<template>
  <div
    class="flex-auto bg-white dark:bg-gray-900 h-screen overflow-y-scroll justify-center"
  >
    <div class="w-full h-full text-gray-900 mt-20">
      <div
        v-if="codeView"
        class="m-8 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500"
      >
        <div
          class="flex justify-center w-full h-12 bg-slate-300 dark:bg-slate-700 rounded-t-lg py-2"
        >
          <button
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      const { blocksList, selectedIcons } = useComponents()
      ...
      watch(
        [() => theme.value, () => selectedIcons.value],
        () => {
          setTimeout(() => {
            generateCodeForPreview()
          }, 100)
