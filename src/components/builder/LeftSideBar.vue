<template>
  <div
    class="w-40 flex-shrink-0 max-w-60 h-screen overflow-y-scroll bg-slate-200 dark:bg-gray-800 py-16"
  >
    <!-- Selected components -->
    <h3 class="px-4 text-sm text-slate-600 dark:text-slate-500">
      Page Sections
    </h3>
    <div class="px-2 my-5 h-32 overflow-y-scroll">
      ...
      >
        ...
        <svg
          @click="removeSelectComponent(index)"
          class="w-3 h-3 cursor-pointer hover:bg-slate-300"
          fill="currentColor"
          viewBox="0 0 20 20"
    ...
    <!-- Icon list -->
    <h3
      class="px-4 pt-3 text-sm text-slate-600 dark:text-slate-500 border-t border-gray-300 dark:border-gray-700"
    >
      Add section
    </h3>
    ...
    setup() {
      ...
      const blockAvailable = (type: string, index: number) => {
        return (
          ...
          blocksList.value[type][index] !== undefined
        )
      }
      ...
      const removeSelectComponent = (val: number) => {
        removeComponent(val)
        setViewType('desktop')
      }

      const handleDragStart = (startElem: any, startIndex: number) => {
